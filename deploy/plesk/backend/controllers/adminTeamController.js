...
function normalizeAdminRole(role) {
  ...
  return trimmedRole.toLowerCase().includes("admin") ? "Admin" : trimmedRole;
}
...
exports.getTeamStats = async (req, res) => {
  try {
    ...
    const [[{ admins }]] = await db.pool.execute(
      "SELECT COUNT(*) as admins FROM admins WHERE LOWER(role) LIKE '%admin%'",
    );
...
exports.updateTeamMember = async (req, res) => {
  try {
    ...
    if (role !== undefined) {
      updates.push("role = ?");
      params.push(normalizeAdminRole(role));
    }
    if (status !== undefined) {
      updates.push("status = ?");
      params.push(status);
